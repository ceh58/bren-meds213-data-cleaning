...
snowsurvey_csv %>%
  count(Snow_cover) %>%
  # Filter for characters that are non-numeric
...
species_long <- species_csv %>%
  pivot_longer(cols = !c(Year, Site, Date, Jdate, Num_observers, All_obs_reported, Observer_hours),
               names_to = "Species",
               values_to = "Species_count",
               values_transform = list(Species_count = as.character)) # coerce into characters
```
